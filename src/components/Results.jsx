...
const Results = () => {
  ...
  return (<section className='w-full sm:w-2/3 flex flex-col items-center justify-between gap-10 md:relative'>
            ...
            <div className='flex flex-col items-center gap-2 md:absolute md:top-1/2 md:-translate-y-1/4'>
              <h1 className='text-white text-6xl md:text-4xl' >YOU {duelResult}</h1>
              <button onClick={()=>setResults(false)} className='w-full bg-white rounded-md py-2 letterSpacing2px shadowClass' style={{color:'var(--darkText)'}} >PLAY AGAIN</button>
            </div>
          </section>
  )
}
