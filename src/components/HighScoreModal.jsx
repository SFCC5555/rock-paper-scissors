...
// HighScoreModal component responsible for displaying high scores and input for nicknames
const HighScoreModal = () => {
    ...
    // State variables for input value and error handling
    ...
    // Function to handle input change and enforce nickname rules
    ...
    // Function to handle the confirmation or closing of the modal
                ...
                <div className='absolute top-0 w-full h-full bg-black/50 flex flex-col gap-3 justify-center items-center z-10'>
                    ...
                    <section className='w-3/4 sm:w-1/2 md:w-1/3 rounded-md px-5 py-7 flex flex-col gap-3' style={{ backgroundColor: 'var(--secondBody)' }}>
                        ...
                        <div className=' flex flex-col border-2 h-full rounded-md px-3 py-5 gap-5' style={{ borderColor: 'var(--headerOutline)' }}>
                            {highScoreList.map((h, i) => (
                                <div key={i + 1} id={i + 1} className={`${h.current ? 'text-white' : 'text-gray-500'} flex justify-between items-end gap-2`}>
                                    <div>{i + 1}.°</div>
                                    <div className='text-sm'>{h.current ? inputValue : h.nickname}</div>
                        ...
                        <div className={`flex items-end ${activeInput ? 'justify-between' : 'justify-end'}`}>
                            {activeInput && (
                                <div className='flex flex-col w-full gap-1 relative'>
                                    ...
                                    <input
                                        ...
                                        id='nickname'
                                        type='text'
                                        className='w-3/4 border-2 rounded-md px-2 py-1 text-xs text-white hover:opacity-70 letterSpacing2px'
                                        style={{ backgroundColor: 'var(--secondBody)', borderColor: error ? 'var(--mainRed)' : 'var(--headerOutline)' }}
                                    />
                            ...
                            <button
                                onClick={handleClick}
                                className='selectButton text-xs rounded-lg border-2 py-1 px-4 letterSpacing2px'
                            >
                                {activeInput ? 'CONFIRM' : 'CLOSE'}
                            </button>
                        </div>
                    </section>
                </div>
...
// Exporting the 'HighScoreModal' component
